...
function parseOptionsFile(optionsPath: string): FlatRouteOption[] {
  ...
  function walk(
    ...
    for (const el of nodes.elements) {
      ...
      // 原始字符串（保留）
      const pathValue = (pathProp.value as types.namedTypes.StringLiteral).value
      const isAbsolute = /^\/+/.test(pathValue)
      // 去掉前导斜杠并拆分成 segments
      const segmentsArr = pathValue.replace(/^\/+/, '').split('/').filter(Boolean)
      const fullPathSegments = [...parentSegments, ...segmentsArr]

      // 保留 AST 属性节点，构建新的 ObjectExpression (除 path 和 children)
...
function applyRouteOptionsToAST(
  ...
  for (const opt of options) {
    ...
    // 找到最长匹配的父节点
    let matchedCount = 0
    let parentNode: types.namedTypes.ObjectExpression | undefined
    for (let i = segments.length; i > 0; i--) {
      const candidate = findObjectInArrayASTByPath(segments.slice(0, i), rootArrayAST)
      if (candidate) {
        matchedCount = i
    ...
    const remainingSegments = segments.slice(matchedCount)

    if (!parentNode) {
      // 没有匹配到父节点，顶层插入
      const newPath = (opt.isAbsolute ? '/' : '') + segments.join('/')
      const newNode = createRouteObjectExpression(newPath, opt.extraProps)
      rootArrayAST.elements.unshift(newNode)
      continue
    }

    // 找到 parent 的 path 属性
    const pathProp = parentNode.properties.find(
      (p): p is types.namedTypes.ObjectProperty =>
        p.type === 'ObjectProperty'
        && p.key.type === 'Identifier'
        && p.key.name === 'path'
    ...
    if (remainingSegments.length > 0) {
      if (pathProp && 'value' in pathProp.value) {
        const val = (pathProp.value as types.namedTypes.StringLiteral).value
        if (typeof val === 'string') {
          const hadLeading = val.startsWith('/')
        ...
        parentNode.properties.unshift(
          types.builders.objectProperty(
            types.builders.identifier('path'),
            types.builders.stringLiteral(newPathValue),
          ),
        )
      }
    }
    ...
    // 合并 extraProps
    mergePropsIntoObject(parentNode, opt.extraProps)
  }
}
...
export async function main() {
  ...
  console.log('Vue Router 路由配置生成器(v1.1) Powered By WIFI连接超时')
